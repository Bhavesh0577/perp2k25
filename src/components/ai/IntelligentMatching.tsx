...
export default function IntelligentMatching() {
  const [matches, setMatches] = useState<TeamMember[]>(DEMO_MEMBERS);
  ...
  const generateMentorshipMatches = () => {
    const mentors = matches.filter(m => m.mentorshipRole === 'mentor');
    const mentees = matches.filter(m => m.mentorshipRole === 'mentee');
    
    const newMatches: MentorshipMatch[] = mentors.map(mentor => ({
      mentor,
      mentees: mentees.filter(mentee => 
        mentor.skills.some(skill => 
          mentee.skills.some(menteeSkill => 
            menteeSkill.name === skill.name && 
            getSkillLevelValue(menteeSkill.level) < getSkillLevelValue(skill.level)
          )
        )
      ),
      focusAreas: mentor.skills.filter(skill => skill.level === 'Expert' || skill.level === 'Advanced').map(s => s.name),
      sessionSchedule: ['Tuesdays 7PM', 'Thursdays 7PM']
    }));

    setMentorshipMatches(newMatches);
  };
  ...
  const analyzeTeamCompatibility = () => {
    ...
    setTimeout(() => {
      setIsAnalyzing(false);
      toast.success('Analysis complete! Updated match scores.');
    }, 3000);
  };
